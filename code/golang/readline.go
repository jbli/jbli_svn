...
func main(){
    ...
    fm = &FileMonitor{
        ...
        last_logline:       "",
        seekJournalPath:    "/tmp/seekJournal.log",
        logfile:            "/data/hekad/log/message-platform/stdout.log",
    ...
    if nil == err {
        ...
        for {
            ...
            if matched,_ := regexp.MatchString("IM_MSG_UPSTREAM", line); matched{
              fmt.Printf("%s",line)
            }
            ...
            fm.seek += int64(len(line))
            IsChanged =true
            //fm.logger.Println("seek_pos:", fm.seek, "last_start:", fm.last_logline_start)
        }
    }
    if IsChanged { 
      fm.updateJournal(1)
